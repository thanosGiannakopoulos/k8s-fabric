...
kubectl hlf ca register --name=ord-ca --user=orderer1 --secret=ordererpw --type=orderer --enroll-id enroll --enroll-secret=enrollpw --mspid=OrdererMSP --namespace=fabric
kubectl hlf ordnode create --storage-class=$SC --enroll-id=orderer1 --mspid=OrdererMSP --enroll-pw=ordererpw --capacity=2Gi --name=ord-node1 --ca-name=ord-ca.fabric --namespace=fabric --image=$ORDERER_IMAGE --version=$ORDERER_VERSION

kubectl wait --timeout=180s --for=condition=Running fabricorderernodes.hlf.kungfusoftware.es -n fabric --all

kubectl hlf ca register --name=ord-ca --user=orderer2 --secret=ordererpw --type=orderer --enroll-id enroll --enroll-secret=enrollpw --mspid=OrdererMSP --namespace=fabric
kubectl hlf ordnode create --storage-class=$SC --enroll-id=orderer2 --mspid=OrdererMSP --enroll-pw=ordererpw --capacity=2Gi --name=ord-node2 --ca-name=ord-ca.fabric --namespace=fabric --image=$ORDERER_IMAGE --version=$ORDERER_VERSION
...
sleep 5s
kubectl hlf ordnode join --block=mychannel.block --name=ord-node2 --namespace=fabric --identity=admin-tls-ordservice.yaml
sleep 5s
